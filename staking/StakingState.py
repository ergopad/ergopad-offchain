...
class StakingState:
    ...
    def addStakeBox(self, stakeBox) -> bool:
        ...
        if not mempool:
            height = stakeBox["inclusionHeight"]
            r5 = stakeBox["additionalRegisters"]["R5"][4:]
            if r5 in self._stakeBoxes:
                if height <= self._stakeBoxes[r5]["inclusionHeight"]:
                    return False
    ...
    def compoundTX(self, appKit: ErgoAppKit, rewardAddress: str):
        try:
            ...
            for box in self._stakeBoxes.values():
                ...
                if (
                    boxR4.apply(0) == emissionR4.apply(1)
                    and not self.mempool.isSpent(box["boxId"])
                    and box["boxId"]
                    != "3230c19953f9a4f1b887f45598ae691c16e5aceda8e319c8d5a80c5d3a5060b3"
                    and box["boxId"]
                    != "86a9b423bfe23b5359466a2b5aa67a9c8e6b1c0f2d9e1b3e4a8d4234f533ed98"
                    and box["boxId"]
                    != "3caba90e59c5d78ecb99ca472cd74f5620c3197c7cf4b3b47ded6d31d93608c7"
                ):
                    ...
                    stakeBoxesOutput.append(
                        appKit.buildOutBox(
                            ...
                            tokens={
                                ...
                                box["assets"][1]["tokenId"]: box["assets"][1]["amount"]
                                + stakeReward,
                            },
                            registers=[
                                ErgoAppKit.ergoValue(
                                    [boxR4.apply(0) + 1, boxR4.apply(1)],
                                ...
                                ErgoAppKit.ergoValue(
                                    (
                                        ...
                                        if "renderedValue"
                                        in box["additionalRegisters"]["R5"]
                                        else box["additionalRegisters"]["R5"][4:]
                                    ),
            ...
            if len(stakeBoxes) == 0:
                return
            emissionAssets = {self.emission["assets"][0]["tokenId"]: 1}
            if totalReward < self.emission["assets"][1]["amount"]:
                emissionAssets[self.emission["assets"][1]["tokenId"]] = (
                    self.emission["assets"][1]["amount"] - totalReward
                )
            ...
            inputs = appKit.getBoxesById(
                [self.emission["boxId"]] + stakeBoxes + [incentiveBox["boxId"]]
            )
            ...
            unsignedTx = appKit.buildUnsignedTransaction(
                inputs=inputs,
                outputs=[emissionOutput]
                + stakeBoxesOutput
                + [incentiveOutput, rewardOutput],
                fee=txFee,
    ...
    def emitTransaction(self, appKit: ErgoAppKit, rewardAddress: str):
        if self.nextCycleTime() < time() * 1000:
            ...
            incentiveInput = appKit.getBoxesById(
                [self.getIncentiveBox(int(5e6))["boxId"]]
            )[0]
            ...
            newStakePoolAmount = self.stakePool["assets"][1][
                "amount"
            ] - stakePoolR4.apply(0)
            dust = 0
            ...
            stakeStateOutput = appKit.buildOutBox(
                ...
                tokens={
                    self.stakeState["assets"][0]["tokenId"]: self.stakeState["assets"][
                        0
                    ]["amount"],
                    self.stakeState["assets"][1]["tokenId"]: self.stakeState["assets"][
                        1
                    ]["amount"],
                },
                registers=[
            ...
            stakePoolOutput = appKit.buildOutBox(
                ...
                tokens={
                    self.stakePool["assets"][0]["tokenId"]: self.stakePool["assets"][0][
                        "amount"
                    ],
                    self.stakePool["assets"][1]["tokenId"]: newStakePoolAmount,
                },
                registers=[stakePoolInput.getRegisters()[0]],
            ...
            emissionOutput = appKit.buildOutBox(
                value=self.emission["value"],
                tokens={
                    self.emission["assets"][0]["tokenId"]: self.emission["assets"][0][
                        "amount"
                    ],
